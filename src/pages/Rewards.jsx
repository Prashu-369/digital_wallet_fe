...
const Rewards = () => {
  const [rewardPoints, setRewardPoints] = useState(500); // Example starting points
  const [message, setMessage] = useState('');
  const [history, setHistory] = useState([]); // Reward history log

  const handleRedeemRewards = (pointsToRedeem) => {
    if (rewardPoints >= pointsToRedeem) {
      setRewardPoints(prev => prev - pointsToRedeem);
      setMessage(`🎉 Successfully redeemed ${pointsToRedeem} points!`);

      // Add entry to history
      setHistory(prev => [
        { type: 'Redeemed', points: pointsToRedeem, timestamp: new Date().toLocaleString() },
        ...prev,
      ]);
    } else {
      setMessage(`❌ Not enough points to redeem ${pointsToRedeem}.`);
  ...
  return (
    ...
    >
      ...
      {/* Reward History */}
      <h3 className="text-xl font-semibold mt-6 mb-3 text-gray-700">📜 Reward History</h3>
      {history.length === 0 ? (
        ...
        <ul className="text-left max-h-64 overflow-y-auto">
          {history.map((entry, index) => (
            <motion.li
              key={index}
              className="mb-2 p-2 bg-gray-50 rounded shadow-sm flex justify-between"
              initial={{ opacity: 0, x: -20 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ delay: index * 0.1 }}
            >
              <span>{entry.type}: {entry.points} points</span>
